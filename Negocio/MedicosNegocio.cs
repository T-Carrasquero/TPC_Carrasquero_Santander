    ...
    public class MedicosNegocio
    {
        ...
        public List<Profesional> listarPorEspecialidad(string especialidad)
        {
            ...
            try
            {
                Conexion.ConnectionString = "data source=.\\SQLEXPRESS; initial catalog=TPC_CLINICA_DB; integrated security=sspi";
                Comando.CommandType = System.Data.CommandType.Text;
                Comando.CommandText = "select medicos.*, e.Descripcion from Medicos medicos left join EspecialidadesPorMedico epm on Dni = epm.DniMedico left join Especialidades e on epm.IdEspecialidad = e.Id where e.Descripcion = '"+ especialidad +"' ";
                Comando.Connection = Conexion;
